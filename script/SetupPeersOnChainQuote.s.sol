...
contract SetupPeersOnChainQuoteScript is Script {
    ...
    function run() public {
        ...
        if (chainId == 11155111) {
            localContract = vm.envAddress("HELLO_WORMHOLE_OC_SEPOLIA");
            remoteContract = vm.envAddress("HELLO_WORMHOLE_OC_BASE_SEPOLIA");
            remoteChainId = 10004; // Base Sepolia Wormhole chain ID
        }
        // Base Sepolia (chainId: 84532) -> setup Sepolia as peer
        else if (chainId == 84532) {
            localContract = vm.envAddress("HELLO_WORMHOLE_OC_BASE_SEPOLIA");
            remoteContract = vm.envAddress("HELLO_WORMHOLE_OC_SEPOLIA");
